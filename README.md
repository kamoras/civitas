...
Current coverage: **50 states configured across 27 strategies.** Some serve many states (`tabular` 14, `certified_table` 4, `clarity` 2, `tally_enr` 2, `totalvote_enr` 2); many are a single state whose election authority genuinely is unlike anyone else's. Five states (MI, NV, NY, OH, OK) have no usable per-state source yet and fall back to `google_civic`, a national source keyed on one fixed, publicly-known address per state — never a visitor's.
