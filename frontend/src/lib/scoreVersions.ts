...
export const SCORE_VERSIONS: ScoreVersion[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      "Legislative Effectiveness compares each member's bill record with the typical (median) member of their chamber, adjusted for whether their party holds the majority. Both of those used to be fixed numbers typed into the code: the median came from a July 2026 measurement, and the majority party came from a table that stopped at the current (119th) Congress. Because bills pile up over a Congress, a fixed median let scores creep up over time, and from January 2027 the majority/minority adjustment would have silently switched off. Both are now measured from the chamber's current members on every update. In the first days of a new Congress, before enough members have introduced bills to measure a typical record, this part of the score shows neutral and says why.",
      "How far above or below the typical member counts as 'the most' is now set by each chamber's own spread of records. It used to be one figure shared by both chambers, which was too wide for the House (where records vary less) and too narrow for the Senate.",
      "Funding is now measured on each member's most recent completed election: the campaign that won them their current seat. It used to include a re-election campaign still under way, so for most of an election year every House member and a third of the Senate were scored on a half-finished campaign, while everyone else was scored on a finished one.",
      "Funding shares (the percentage from PACs, the percentage from small donors) are now shares of contributions: money given by individuals, PACs, party committees or the candidate. They used to be shares of total receipts, which also count transfers from joint fundraising committees. Those transfers were counted in the total, but their PAC content was never broken out, so members who raise heavily through joint fundraising committees looked less PAC-reliant than they are. Total raised is still shown as before.",
      "The donor and industry detail now covers the same period as the totals it's compared with: six years for senators and two for House members. It used to cover four years for both, which cut off the start of a Senate campaign and pulled in a House member's previous campaign.",
      "The PAC-share score is measured against the median member of the member's own chamber. That median used to be a fixed number typed into the code; it is now re-measured on every update.",
      "Legislative leadership: the lowest-ranked member in each chamber scored exactly 0 on the leadership network measure, and the code read that 0 as 'no data' and gave them a neutral 50. The next member up scored close to 0. A 0 now counts as the real score it is. Separately, a cosponsored bill with no information about how far it got used to count as if it had become law; it now counts as a typical bill.",
      "Several fixed numbers in the formulas were measurements someone had once taken and typed into the code. Each is now re-measured from the current members on every update: the typical PAC dollar total used when a PAC can't be identified, what counts as a normal concentration of top donors, the typical House member's small-donor share (House members used to be judged against a flat 40%, which put the typical member below neutral), and how much more often majority-party bills advance than minority-party ones. Two scales changed slightly as a result: top-donor concentration and the House small-donor share are now centred on the chamber's own typical member.",
      "Senate bills are now sorted into how far they got (committee action, passed the Senate, became law) before scoring. They used to be sorted afterwards, so the Senate was scored with a cruder rule that missed, for example, a bill that passed the Senate and then went to a House committee. The House was already done in the right order.",
      "Constituent Alignment was rebuilt on evidence from 2,545 U.S. House re-election results (1994-2010): each design choice was kept only if it helped predict how incumbents did with their own voters once the district's partisanship and the national tide were accounted for. Four changes followed. Voting with the party more than members of the same party in similar seats now lowers the score; it used to be held at neutral, but in the test it was the strongest signal of all. Breaking from the party, and holding a position nearer the seat's center, now earn the same credit in safe seats as in competitive ones; the test found voters in safe seats responded just as much. The break rate a seat 'expects' is now measured each update from how the chamber's own members in similar seats vote, separately for each party; it used to be a hand-set curve that expected too little crossing from members in seats that lean to the other party (20% expected, where the 2004 House showed about 25%). Voting position now uses the congress-specific Nokken-Poole estimate rather than career-long DW-NOMINATE, which covers the current term rather than the career and predicted results better. The full study is in docs/research/constituent-alignment.md.",
      "Funding Independence counts each signal once. Checked against FEC's own data for 2020-2024 incumbents, two inputs were removed. Outside spending (super PAC and other independent spending supporting a member) used to count half as much as a PAC contribution. It tracks how competitive the race is, not how dependent the member is: it was highest in swing seats, lower for members who take more PAC money, and the member cannot legally direct it. For the average senator in a seat within 3 points of even, it was taking about 28 points off the PAC part of the score. 'Source breadth' was the small-donor share counted a second time (it explained 79-86% of breadth's variation), plus a penalty on candidates' own money. A third copy, crediting small donors again when too little money could be assigned to an industry, now counts as neutral. The remaining parts keep their relative weights. The study is in docs/research/funding-independence.md.",
      "The leaderboard's week-over-week arrows now compare only scores from the same scoring method and the same Congress. They used to compare with any score from a week earlier, so the day this update ships every member would have shown a rise or fall that was really the change of method. Where there is no comparable earlier score, the arrow shows a dash.",
      "The partisan-depth label on senator cards (deep, moderate, centrist) is now relative to the senator's own party: the most partisan third, the middle third and the least partisan third. It used to use one fixed cut-off for both parties, tuned so particular named senators landed in chosen bands, even though the two parties' leans sit on different ranges. For senators with few votes, the cosponsorship-based estimate that fills in is now put on the same scale as vote-based leans first; it used to be able to push them to the extreme. This label is shown, not scored.",
      "Supreme Court scorecards now have two measures instead of four. 'Judicial restraint' scored how often a justice dissents; tested on the Rehnquist Court's 1994-2004 votes, dissent frequency tracks distance from the Court's median justice (0.83 correlation), and in a simulated 6-3 Court it scored the smaller bloc about 21 points lower for being outvoted, so it was removed. 'Bipartisan agreement' measured the same thing as independence (0.86 correlation), so its weight joined independence. The study is in docs/research/justice-scores.md.",
      "Constituent Alignment for Independents: the score breakdown now scores them against the party they caucus with, as the score itself always has. It used to score them as belonging to neither party, so the breakdown and the score could disagree.",
      "Legislative Effectiveness now compares each member's bill record with the typical member of the same status (majority or minority) in their chamber, as Volden & Wiseman's benchmarks do. It used to compare with the chamber's typical member, scaled by how much more often majority bills advance. Most credit comes from introducing bills, which majority status doesn't change, so that scaling over-corrected: in a simulated House where every member performed exactly at their status's rate, the typical minority member scored 97 and the typical majority member 7.5, and on production data the House gap between the parties reached 18 points. Each status's typical member now scores 50 on this part.",
    ],
  },
  {
    ...
    changes: [
      ...
      "GDP growth used a fixed curve built around a 'post-WWII average of 3.2%' and applied it to every president back to 1789. It is now compared with other presidents in the same era, before and after 1947. Growth estimates from before the war exaggerate booms and busts (Romer 1989), and in the Maddison Project's figures term growth varied two and a half times as much then; the fixed curve pinned one in eight earlier presidents at 0 or 100 and no postwar president at either.",
      "Jobs created is compared with the measured average for presidents since 1939 instead of a fixed 3 million jobs a year. It stays in absolute jobs: the percentage rate falls over time with the workforce's own slower growth, so it would have penalized recent presidents for demographics.",
      "Agency Alignment no longer rewards the number of rules issued. Rule counts follow an administration's view of regulation (the Federal Register's record low was 2019, its record high 2024), so more rules scoring higher amounted to a policy preference. It now measures the share of rulemaking carried through to a final rule, compared with other administrations since 1994.",
      "The 'show the math' panel for Effectiveness could use a stored adjusted-GDP figure that no update had written since the GDP calculation moved; it now uses the same figure the score does.",
      "Not changed, but disclosed: since 1946, 60% of the variation in a president's term growth is shared with other advanced economies over the same years, so Effectiveness mostly reflects the economy a president presided over. Comparing growth with peer economies would remove that, but needs a data source the pipeline does not have yet. The study is in docs/research/president-scores.md.",
    ],
  },
  {
    version: "v6.12",
  ...
  },
  {
    ...
    date: "2026-07-23",
    title:
      "Coalition breadth moves to Legislative Effectiveness; Constituent Alignment gains a real position signal",
    ...
    changes: [
      ...
      "Follow-up on a limitation disclosed since v6.4: Funding Diversity's industry-concentration index (HHI) was flagged as possibly under-weighting a donor base's single most-dominant contributor relative to a top-share measure, the same reason income-inequality research often prefers top-1%-share over Gini. Tested directly against every scorecarded member's live industry data (n=413): HHI and top-1-industry-share rank members at rho=0.94 — near-total agreement, not a coincidence. The theoretical concern is about raw per-donor amounts, which really are long-tailed like income; this component buckets by industry first, and that aggregation already smooths away the extreme tail a top-share measure exists to handle. No score change — HHI stays, now on tested grounds rather than a disclosed open question.",
    ],
  },
  {
    version: "v6.10",
  ...
  },
  {
    ...
    date: "2026-07-22",
    title:
      "Historical Legacy's weight held constant — it was never actually 35% for most presidents",
  ...
  },
  {
    ...
    date: "2026-07-19",
    title:
      "Funding Diversity folded into Funding Independence; Donor Independence removed from Constituent Alignment",
    changes: [
  ...
  },
  {
    ...
    date: "2026-07-19",
    title:
      "Academic-fidelity audit: citations corrected, Legislative Effectiveness rebuilt to match its own methodology",
    changes: [
  ...
  },
  {
    ...
    date: "2026-07-16",
    title:
      "Funding Diversity's small-donor ceiling raised for overwhelmingly grassroots-funded members",
    changes: [
