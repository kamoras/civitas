...
export const SCORE_VERSIONS: ScoreVersion[] = [
  {
    ...
    changes: [
      "A fairness review asked whether it is fair to score a senator down for \"sticking with their party too much.\" Under v4.2, a member who voted with their party more loyally than their seat's partisan lean \"expected\" was scored below neutral for it — a swing- or opposed-seat loyalist could lose ~14 points purely for a low party-defection rate. Checked against the political-science literature on representation, that isn't supported: a low defection rate is not readable evidence of failing to represent constituents. It may be faithful representation of the coalition that actually elected the member (Fenno 1978; Bishin 2009; Clinton 2006); ~90%+ party-line voting is now the structural norm for both parties even in competitive states (Levendusky 2009; Hopkins 2018), so it carries little individual signal; and being \"out of step\" is a matter of ideological position relative to the district, not a loyalty rate (Canes-Wrone, Brady & Cogan 2002; Krehbiel 2000). Below-expected loyalty now floors at neutral (50) instead of dropping below it.",
      "This version is governed by one explicit principle so the two changes cohere rather than borrowing a different theory for each: the score moves off neutral only where there is readable evidence a member is representing their constituents, and treats behavior whose meaning can't be read as neutral. Party loyalty is unreadable (it could be faithful or not — we can't tell from the rate), so it sits at neutral, neither penalized nor rewarded.",
      "The readable side is crossing that plausibly moves toward the seat's political center — but that was previously credited by RATE, blind to DIRECTION, even though the members who defect most are often ideological extremists breaking from their own flank (Rand Paul from the right, Bernie Sanders from the left), not moderates moving to the center (Kirkland & Slapin 2017). Above-expected crossing now carries a directional discount based on the member's own ideological position (from the platform's existing party-blind cosponsorship measure): a moderate-wing crosser keeps full credit, an extreme-flank crosser earns less. A flank defector is not penalized — just credited less. A member with no ideology data is never discounted.",
      "Disclosed limitation, not fixed here: this dimension measures the rate and direction of a member's deviation from their party, not the distance between the member's position and their constituency's. So it can reward visible crossing toward the center and it no longer punishes loyalty, but it cannot positively credit representation achieved through congruent loyalty — a member whose party already matches a lopsided state scores 50, not high, even if perfectly aligned. Measuring that directly (positional congruence) is the real long-term direction; it needs a party-relative target and careful calibration, so it's tracked as follow-up rather than done in this pass. The strength of the new flank discount is likewise a research-informed starting value to be tuned against live data.",
    ],
  },
  {
    version: "v6.5",
