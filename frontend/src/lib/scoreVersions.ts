...
export const SCORE_VERSIONS: ScoreVersion[] = [
  {
    ...
    changes: [
      ...
      "When position congruence is active it replaces the v6.7 position-mismatch discount (which approximated the same construct from cosponsorship patterns — a signal v6.8 found mechanically entangled with bipartisanship); applying both would double-count, the exact failure v6.8 fixed. The old discount remains as the fallback for members the ideal-point data doesn't cover. Every number the new component uses (per-member positions, the seat-conditional fits, the saturation scale) is computed from real data by a checked-in generator script with ingestion gates, mirroring how seat partisan-lean data is already ingested — none of it is hand-picked. Until that generated data file is present, the component is simply skipped and the dimension runs on seat-relative vote alignment alone; ground-truth ranges and the population-spread floor get re-verified on the first pipeline run with it active, per the standing convention for any change to this dimension.",
    ],
  },
  {
    version: "v6.10",
