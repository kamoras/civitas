...
export const SCORE_TERMS: Record<ScoreKey, ScoreTerm> = {
  ...
  independentVoting: {
    ...
    description:
      "Does their voting match what their state elected them to do? Each member's party-line break rate is compared to what their seat's partisan lean predicts. Party loyalty is never itself a penalty — voting the party line is how you represent the coalition that elected you, so below-expected defection sits at neutral, not below it. Crossing party lines earns extra credit only where it plausibly moves toward the state's political center (crediting defection more where the seat leans the other way). Defection is not a virtue by itself.",
  },
  fundingDiversity: {
