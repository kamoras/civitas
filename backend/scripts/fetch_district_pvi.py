...
Superseded as the primary data path by app/pipeline/fetch/district_pvi.py
(2026-07), which runs this same fetch/parse/gate logic automatically
inside the Supplementary pipeline (weekly, or immediately if the
persistent volume has no data yet) and writes to /data/ rather than these
bundled files. This script still exists to regenerate the bundled
app/data/*.json fallback below — the copy served only in the narrow
window before a fresh deployment's first automated ingest completes.

Regenerates app/data/district_pvi.json, which _calc_constituent_alignment
uses as the seat expectation for House members (senators use the
state-level state_pvi.json / _state_pvi, generated by fetch_state_pvi.py).
...
0 = EVEN), same sign convention as state_pvi.json in score_calculator.py.
