...
representation dimension.
  ...
  2. Position congruence ADDED to Constituent Alignment (30% when data
     ...
     saturation) lives in generated data (app/data/member_ideal_points.
     json, scripts/fetch_member_ideal_points.py, ingestion-gated like
     fetch_state_pvi.py) — no Python-side magnitude constants, honoring
     the no-un-fit-constants rule; until the file is generated the
     component is INERT (skipped, weight renormalized) and this
     dimension runs on the vote component alone.
...
# loyal-branch position-mismatch discount when active; inert until
# app/data/member_ideal_points.json is generated
# (scripts/fetch_member_ideal_points.py). See the top-of-file "Changes
# from v6.10 -> v6.11" note for the full account.
...
def _member_ideal_points(chamber: str) -> dict:
    ...
    Ingested from app/data/member_ideal_points.json, generated by
    scripts/fetch_member_ideal_points.py from Voteview's published
    DW-NOMINATE member estimates (Lewis et al., voteview.com) joined
    against this repo's own seat-lean tables (state_pvi.json /
    district_pvi.json) — same generated-data convention as
    state_pvi.json/small_donor_baseline.json: every number the scoring
    formula consumes (positions, per-party regression coefficients,
    saturation scale) is computed from real data by the script, never
    hand-typed here. Regenerate once per congress (or after Voteview's
    weekly updates meaningfully shift estimates) by rerunning the script
    ...
    seat-relative vote component alone. Missing data is never punitive —
    same convention as every other loader in this file. This also means
    the component ships INERT until the generated file exists (the
    CROSSING_QUALITY_DISCOUNT posture: no un-fit constants — here there
    are no Python-side magnitude constants at all; they all live in the
    generated JSON).
    """
    ...
    if _member_ideal_points_cache is None:
        import json
        import pathlib
        path = pathlib.Path(__file__).resolve().parent.parent.parent / "data" / "member_ideal_points.json"
        try:
            _member_ideal_points_cache = json.loads(path.read_text())
        except Exception:
            logger.warning(
                "member_ideal_points.json unavailable — position-congruence "
                "component will be skipped for every member; generate with "
                "scripts/fetch_member_ideal_points.py"
            )
...
def _seat_pvi(state: str, district: int | None = None) -> int:
    ...
    seat-conditional regression (which was FIT on this same raw scale by
    scripts/fetch_member_ideal_points.py) are derived from — one lookup,
...
def _calc_constituent_alignment(
    ...
    Components:
      ...
      2. Position congruence (30%, when roll-call ideal-point data
         ...
         position on seat PVI, fit from real data by
         scripts/fetch_member_ideal_points.py. This measures the actual
